...
func (dbm *DBManager) SetDMLBatchSize(n int) { if n > 0 { dbm.dmlBatchSize = n } }
...
func (dbm *DBManager) InitDB(dropIfExists bool) {
    ...
    if dropIfExists {
        ...
        if err != nil { log.Printf("Error dropping database: %v", err) }
        _, err = dbm.db.Exec(fmt.Sprintf("CREATE DATABASE IF NOT EXISTS %s", dbName))
        if err != nil { log.Printf("Error creating database: %v", err) }
    }
...
func (dbm *DBManager) DisableAutoAnalyze() {
    ...
    if err != nil { fmt.Printf("    [Warning] Failed to disable auto analyze: %v\n", err) }
}
...
func (dbm *DBManager) CreateTable(tableName string, df Frame) {
    cols := []string{}
    indexes := []string{}
    ...
    ddl := fmt.Sprintf(`CREATE TABLE IF NOT EXISTS %s (
        ...
        tableName, strings.Join(cols, ", "), strings.Join(indexes, ", "))
    fmt.Println(ddl)
    _, err := dbm.db.Exec(fmt.Sprintf("DROP TABLE IF EXISTS %s", tableName))
    if err != nil { log.Printf("Error dropping table: %v", err) }
    _, err = dbm.db.Exec(ddl)
    if err != nil { log.Printf("Error creating table: %v", err) }
...
func (dbm *DBManager) inferSQLType(df Frame, colName string) string {
    ...
    if len(col) == 0 { return "VARCHAR(255)" }
    for _, val := range col {
        if val != nil {
            switch val.(type) {
...
func (dbm *DBManager) LoadDataInfile(tableName string, csvPath string) {
    ...
    if err != nil { log.Printf("Error getting absolute path: %v", err); return }

    f, err := os.Open(absPath)
    if err != nil { log.Printf("    [Error] Failed to open CSV to read header: %v", err); return }
    csvReader := csv.NewReader(f)
    header, err := csvReader.Read()
    f.Close()
    if err != nil { log.Printf("    [Error] Failed to read CSV header: %v", err); return }

    quotedCols := make([]string, len(header))
    for i, col := range header { quotedCols[i] = fmt.Sprintf("`%s`", strings.TrimSpace(col)) }
    ...
    if _, err := dbm.db.Exec(sql); err != nil { log.Printf("    [Error] Load Data failed: %v", err); return }
...
func (dbm *DBManager) GetSingleTableHealth(tableName string) int {
    ...
    var healthy int
    if rows.Next() {
        if err := rows.Scan(&dbNameResult, &tableNameResult, &partition, &healthy); err == nil { return healthy }
    }
    return 0
}
...
func (dbm *DBManager) AnalyzeAllTablesInDB(dbName string) {
    if strings.TrimSpace(dbName) == "" { return }
    fmt.Printf("    [DB] Analyzing all tables in database: %s\n", dbName)
    if _, err := dbm.db.Exec(fmt.Sprintf("USE `%s`", dbName)); err != nil { fmt.Printf("    [Error] Failed to switch to DB %s: %v\n", dbName, err); return }
    tblRows, err := dbm.db.Query("SHOW TABLES")
    if err != nil { fmt.Printf("    [Error] Failed to list tables in %s: %v\n", dbName, err); return }
    defer tblRows.Close()
    var tables []string
    for tblRows.Next() { var t string; if err := tblRows.Scan(&t); err == nil { tables = append(tables, t) } }
    maxRetries, interval := dbm.getAnalyzeParams()
    for _, t := range tables {
        fmt.Printf("    [DB] ANALYZE TABLE %s.%s ALL COLUMNS ...\n", dbName, t)
        startTime := time.Now()
        if _, err := dbm.db.Exec(fmt.Sprintf("ANALYZE TABLE `%s` ALL COLUMNS", t)); err != nil { fmt.Printf("      [Error] Analyze failed for %s.%s: %v\n", dbName, t, err); continue }
        for i := 0; i < maxRetries; i++ {
            health := dbm.GetSingleTableHealthInDB(dbName, t)
            if health == 100 { break }
            time.Sleep(interval)
            if i == maxRetries-1 { fmt.Printf("      [Warning] Stats health for %s.%s reached %d%%, timeout.\n", dbName, t, health) }
        }
...
func (dbm *DBManager) AnalyzeTable(tableName string) {
    ...
    startTime := time.Now()
    if _, err := dbm.db.Exec(fmt.Sprintf("ANALYZE TABLE %s ALL COLUMNS", tableName)); err != nil { fmt.Printf("    [Error] Analyze failed: %v\n", err); return }
    ...
    for i := 0; i < maxRetries; i++ {
        ...
        if health == 100 { break }
        time.Sleep(interval)
        if i == maxRetries-1 { fmt.Printf("    [Warning] Stats health reached %d%%, timed out waiting for 100%%.\n", health) }
    }
    duration := time.Since(startTime)
...
func (dbm *DBManager) ExecuteSQLFile(sqlPath string) {
    ...
    if _, err := os.Stat(sqlPath); os.IsNotExist(err) { return }
    content, err := os.ReadFile(sqlPath)
    if err != nil { log.Printf("Error reading SQL file: %v", err); return }
    _, err = dbm.db.Exec("SET tidb_mem_quota_query = 2 * 1024 * 1024 * 1024")
    if err != nil { fmt.Printf("    [Warning] Failed to increase memory quota: %v\n", err) }
    _, _ = dbm.db.Exec("SET GLOBAL local_infile = 1")
    ...
    commitTx := func() {
        if tx != nil {
            if err := tx.Commit(); err != nil { log.Printf("Error committing transaction: %v", err) }
            tx = nil
        }
    }
    for _, statement := range statements {
        ...
        if tx == nil {
            var err error
            tx, err = dbm.db.Begin()
            if err != nil { log.Printf("Error starting transaction: %v", err); return }
        }
        ...
        if strings.Contains(up, "LOAD DATA LOCAL INFILE") {
            if i := strings.Index(up, "INFILE"); i >= 0 {
                ...
                if j := strings.IndexByte(rest, '\''); j >= 0 {
                    rest2 := rest[j+1:]
                    if k := strings.IndexByte(rest2, '\''); k >= 0 { mysql.RegisterLocalFile(rest2[:k]) }
                }
            }
        }
...
func (dbm *DBManager) ExecuteAndExplain(queryFile string) []itypes.QueryResult {
    if _, err := os.Stat(queryFile); os.IsNotExist(err) { return []itypes.QueryResult{} }
    content, err := os.ReadFile(queryFile)
    if err != nil { log.Printf("Error reading query file: %v", err); return []itypes.QueryResult{} }
    queries := strings.Split(string(content), ";")
    results := []itypes.QueryResult{}
    queryID := 1
    for _, query := range queries {
        query = strings.TrimSpace(query)
        if query == "" || strings.HasPrefix(query, "--") { continue }
        start := time.Now()
        rows, err := dbm.db.Query(query)
        if err != nil { fmt.Printf("      Q%d Error: %v\n", queryID, err); queryID++; continue }
        rows.Close()
        duration := time.Since(start).Milliseconds()
        explainQuery := fmt.Sprintf("EXPLAIN ANALYZE %s", query)
        explainRows, err := dbm.db.Query(explainQuery)
        if err != nil { fmt.Printf("      Explain Error: %v\n", err); queryID++; continue }
        ...
        for i, col := range columns {
            ...
            if strings.EqualFold(col, "estRows") { estRowIdx = i }
            if strings.EqualFold(col, "actRows") { actRowIdx = i }
        }
        for i, col := range columns { sb.WriteString(col); if i < count-1 { sb.WriteString("\t") } }
        sb.WriteString("\n")
        for explainRows.Next() {
            if err := explainRows.Scan(valuePtrs...); err != nil { continue }
            for i, val := range values {
                var v interface{}
                if b, ok := val.([]byte); ok { v = string(b) } else { v = val }
                sb.WriteString(fmt.Sprintf("%v", v))
                if i < count-1 { sb.WriteString("\t") }
            }
            sb.WriteString("\n")
            if estRowIdx != -1 && actRowIdx != -1 {
                ...
                if errRatio > maxErrorRatio { maxErrorRatio = errRatio; maxErrorValue = errVal }
                if errRatio >= 10 && errVal >= 1000 { riskCount++ }
            }
        }
        explainRows.Close()
...
func (dbm *DBManager) ExecuteAndExplainQueriesOnDB(targetDB string, queries []string) []itypes.QueryResult {
    results := []itypes.QueryResult{}
    if targetDB == "" { return results }
    if _, err := dbm.db.Exec(fmt.Sprintf("USE `%s`", targetDB)); err != nil { fmt.Printf("      [Warning] Cannot switch to DB %s: %v\n", targetDB, err); return results }
    queryID := 1
    for _, query := range queries {
        q := strings.TrimSpace(query)
        if q == "" || strings.HasPrefix(q, "--") { continue }
        start := time.Now()
...
func (dbm *DBManager) GetTableStats(tableName string, columns []string) map[string]map[string]interface{} {
    ...
    for _, col := range columns {
        ...
        if err := dbm.db.QueryRow(query).Scan(&minVal, &maxVal); err != nil { fmt.Printf("      [Warning] Failed to fetch stats for %s.%s: %v\n", tableName, col, err); continue }
        if minVal.Valid { stats[col]["min"] = minVal.String }
        if maxVal.Valid { stats[col]["max"] = maxVal.String }
    }
    return stats
}
...
func (dbm *DBManager) GetStatsHealthy() map[string]int {
    ...
    if err != nil { fmt.Printf("Warning: Could not execute SHOW STATS_HEALTHY: %v\n", err); return statsHealthy }
    defer rows.Close()
    var dbName, tableName string
    var healthy int
    for rows.Next() {
        var partName sql.NullString
        if err := rows.Scan(&dbName, &tableName, &partName, &healthy); err != nil { continue }
...
func (dbm *DBManager) GetRandomIDs(tableName string, n int) []int64 {
    ...
    for rows.Next() {
        ...
        if err := rows.Scan(&id); err == nil && id.Valid { res = append(res, id.Int64) }
    }
    return res
}

// Close closes the database connection
func (dbm *DBManager) Close() { if dbm.db != nil { dbm.db.Close() } }
...
func truncateString(s string, maxLen int) string { if len(s) <= maxLen { return s }; if maxLen <= 3 { return s[:maxLen] }; return s[:maxLen-3] + "..." }
...
func extractQueryLabel(sqltxt string) string {
    ...
    if idx := strings.Index(upper, "/* LABEL:"); idx >= 0 {
        ...
        if j := strings.Index(urest, "LABEL:"); j >= 0 {
            ...
            if k := strings.Index(after, "*/"); k >= 0 { after = after[:k] }
