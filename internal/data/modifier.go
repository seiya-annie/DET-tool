...
func (dm *DataModifier) Apply(df *DataFrame, modelConfig map[string]interface{}, tableName string, logger SQLLogger) (*DataFrame, ApplyStats) {
    ...
    if inc == nil { return df, ApplyStats{BaseRows: len(df.data)} }
...
func (dm *DataModifier) cloneDataFrame(df *DataFrame) *DataFrame { cloned := &DataFrame{columns: make([]string, len(df.columns)), data: make([][]interface{}, len(df.data))}; copy(cloned.columns, df.columns); for i, row := range df.data { cloned.data[i] = make([]interface{}, len(row)); copy(cloned.data[i], row) }; return cloned }
...
func (dm *DataModifier) applyInserts(df *DataFrame, modelConfig map[string]interface{}, tableName string, insertRows int, logger SQLLogger) *DataFrame {
    ...
    if p, ok := modelConfig["Params"].(map[string]interface{}); ok { for k, v := range p { cfg["Params"].(map[string]interface{})[k] = v } }
    ...
    if logger != nil {
        ...
        if dm.insertMode == "load" {
            ...
            if err := newData.SaveCSV(csvName); err == nil { logger.LogLoadDataLocalInfile(tableName, newData, abs) } else { logger.LogInsertBatch(tableName, newData, dm.insertBatchSz) }
        } else { logger.LogInsertBatch(tableName, newData, dm.insertBatchSz) }
    }
...
func (dm *DataModifier) applyUpdates(df *DataFrame, modelConfig map[string]interface{}, tableName string, updateRatio float64, logger SQLLogger) (*DataFrame, int) {
    ...
    updateCount := int(float64(len(valid)) * updateRatio); if updateCount == 0 { updateCount = 1 }; if updateCount > len(valid) { updateCount = len(valid) }
    selected := make([]int, updateCount); used := make(map[int]bool)
    for i := 0; i < updateCount; i++ { for { idx := valid[dm.rng.Intn(len(valid))]; if !used[idx] { selected[i] = idx; used[idx] = true; break } } }
    cfg := map[string]interface{}{"Name": modelConfig["Name"], "Type": modelConfig["Type"], "Params": map[string]interface{}{}, "Incremental": map[string]interface{}{}}
    if p, ok := modelConfig["Params"].(map[string]interface{}); ok { for k, v := range p { cfg["Params"].(map[string]interface{})[k] = v } }
    ...
    for i := 0; i < updateCount && i < len(ids) && i < len(selected) && i < len(updateData.data); i++ {
        idx := selected[i]; updateRow := updateData.data[i]
        for j, colName := range columnNames { colIndex := getColumnIndex(df, colName); if colIndex != -1 && colIndex < len(df.data[idx]) && j < len(updateRow) { df.data[idx][colIndex] = updateRow[j] } }
        tmpRow := make([]interface{}, 1+len(df.columns)); tmpRow[0] = ids[i]; copy(tmpRow[1:], df.data[idx]); tmp.AddRow(tmpRow)
    }
    if logger != nil && tmp.Size() > 0 { logger.LogComment(fmt.Sprintf("UPDATE operations for %s (temp table join)", tableName)); tmpName := fmt.Sprintf("tmp_%s_update", tableName); logger.LogCreateTempTable(tmpName, tmp, "id"); csvName := filepath.Join(dm.tmpDir, fmt.Sprintf("inc_update_%s.csv", tableName)); abs, _ := filepath.Abs(csvName); if err := tmp.SaveCSV(csvName); err == nil { logger.LogLoadDataLocalInfile(tmpName, tmp, abs); logger.LogUpdateFromTempJoin(tableName, tmpName, "id", columnNames); logger.LogDropTempTable(tmpName) } }
...
func (dm *DataModifier) applyDeletes(df *DataFrame, tableName string, deleteRatio float64, logger SQLLogger) (*DataFrame, int) {
    if deleteRatio <= 0 || deleteRatio > 1 { return df, 0 }
    currentTotal := len(df.data); if currentTotal == 0 { return df, 0 }
    deleteCount := int(float64(currentTotal) * deleteRatio); if deleteCount == 0 { deleteCount = 1 }; if deleteCount > currentTotal { deleteCount = currentTotal }
    if logger != nil { logger.LogComment(fmt.Sprintf("DELETE operations for %s", tableName)); if dm.dbm != nil { ids := dm.dbm.GetRandomIDs(tableName, deleteCount); if len(ids) > 0 { idsIfc := make([]interface{}, len(ids)); for i, v := range ids { idsIfc[i] = v }; logger.LogDeleteByIDs(tableName, "id", idsIfc, 1000) } else { logger.LogDeleteLimit(tableName, deleteCount) } } else { logger.LogDeleteLimit(tableName, deleteCount) } }
    if deleteCount < currentTotal {
        deleteIndices := make(map[int]bool)
        for len(deleteIndices) < deleteCount { idx := dm.rng.Intn(currentTotal); deleteIndices[idx] = true }
        for idx := range deleteIndices { if idx < len(df.data) { for j := range df.data[idx] { df.data[idx][j] = nil } } }
    } else { for i := range df.data { for j := range df.data[i] { df.data[i][j] = nil } } }
