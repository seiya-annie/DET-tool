...
func (df *DataFrame) GetColumn(name string) []interface{} {
    colIndex := -1
    for i, col := range df.columns { if col == name { colIndex = i; break } }
    if colIndex == -1 { return nil }
    result := make([]interface{}, len(df.data))
    for i, row := range df.data { if colIndex < len(row) { result[i] = row[colIndex] } }
    return result
}

func (df *DataFrame) RenameColumns(renameMap map[string]string) {
    for i, col := range df.columns { if newName, ok := renameMap[col]; ok { df.columns[i] = newName } }
}
...
func (df *DataFrame) SaveCSV(filename string) error {
    ...
    defer writer.Flush()
    if err := writer.Write(df.columns); err != nil { return err }
    for _, row := range df.data {
        stringRow := make([]string, len(row))
        for i, val := range row {
            if val == nil { stringRow[i] = "" } else { stringRow[i] = fmt.Sprintf("%v", val) }
        }
        if err := writer.Write(stringRow); err != nil { return err }
    }
    return nil
}
...
func LoadDataFrameFromCSV(filename string) (*DataFrame, error) {
    ...
    for {
        ...
        for i, val := range record {
            if val == "" || val == "<nil>" { row[i] = nil; continue }
            if intVal, err := strconv.Atoi(val); err == nil { row[i] = intVal
            } else if floatVal, err := strconv.ParseFloat(val, 64); err == nil { row[i] = floatVal
            } else if timeVal, err := time.Parse("2006-01-02", val); err == nil { row[i] = timeVal
            } else { row[i] = val }
        }
...
func Concat(dfs []*DataFrame, ignoreIndex bool) *DataFrame {
    if len(dfs) == 0 { return NewDataFrame() }
    result := &DataFrame{columns: make([]string, len(dfs[0].columns)), data: make([][]interface{}, 0)}
    copy(result.columns, dfs[0].columns)
    for _, df := range dfs { result.data = append(result.data, df.data...) }
    return result
}

func IsNull(val interface{}) bool { if val == nil { return true }; if s, ok := val.(string); ok && s == "" { return true }; return false }
...
func ColumnToStringSlice(df *DataFrame, columnName string) []string {
    col := df.GetColumn(columnName); if col == nil { return nil }
    result := make([]string, len(col)); for i, v := range col { result[i] = fmt.Sprintf("%v", v) }; return result
}
func ColumnToIntSlice(df *DataFrame, columnName string) []int {
    col := df.GetColumn(columnName); if col == nil { return nil }
    result := make([]int, len(col))
    for i, val := range col {
        switch v := val.(type) { case int: result[i] = v; case float64: result[i] = int(v); case string: if iv, err := strconv.Atoi(v); err == nil { result[i] = iv } }
    }
    return result
}

func StringInSlice(str string, list []string) bool { for _, v := range list { if v == str { return true } }; return false }
func IntInSlice(val int, list []int) bool { for _, v := range list { if v == val { return true } }; return false }

func ParseDateRange(dateStr string) (time.Time, error) {
    formats := []string{"2006-01-02", "2006-01-02 15:04:05", "01/02/2006", "Jan 2, 2006"}
    for _, f := range formats { if t, err := time.Parse(f, strings.TrimSpace(dateStr)); err == nil { return t, nil } }
    return time.Time{}, fmt.Errorf("unable to parse date: %s", dateStr)
}

func getColumnIndex(df *DataFrame, columnName string) int {
    for i, col := range df.columns { if col == columnName { return i } }
    return -1
}

func (df *DataFrame) Size() int { if df == nil { return 0 }; return len(df.data) }
