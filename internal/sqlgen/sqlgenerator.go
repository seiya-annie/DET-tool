...
func (sg *SqlGenerator) LogDeleteLimit(tableName string, limitCount int) {
    if limitCount > 0 { sg.statements = append(sg.statements, fmt.Sprintf("DELETE FROM `%s` LIMIT %d;", tableName, limitCount)) }
}
...
func (sg *SqlGenerator) LogDeleteByIDs(tableName, idColumn string, ids []interface{}, batchSize int) {
    ...
    toSQL := func(v interface{}) string {
        switch x := v.(type) { case string: return fmt.Sprintf("'%s'", strings.ReplaceAll(x, "'", "''")); default: return fmt.Sprintf("%v", x) }
    }
    for i := 0; i < len(ids); i += batchSize {
        end := i + batchSize; if end > len(ids) { end = len(ids) }
        vals := make([]string, end-i)
        for j := i; j < end; j++ { vals[j-i] = toSQL(ids[j]) }
...
func (sg *SqlGenerator) LogUpdate(tableName, idColumn string, ids []interface{}, columnNames []string, values [][]interface{}) {
    for i, id := range ids {
        ...
        for j, colName := range columnNames { if j < len(values[i]) { sets = append(sets, fmt.Sprintf("`%s`=%s", colName, sg.formatValue(values[i][j]))) } }
        if len(sets) > 0 { sg.statements = append(sg.statements, fmt.Sprintf("UPDATE `%s` SET %s WHERE `%s`=%s;", tableName, strings.Join(sets, ", "), idColumn, sg.formatValue(id))) }
    }
}

func (sg *SqlGenerator) LogInsertBatch(tableName string, df Frame, rows [][]interface{}, columns []string, batchSize int) {
    if len(columns) == 0 || len(rows) == 0 { return }
    if batchSize <= 0 { batchSize = 1000 }
    colList := strings.Join(columns, ", ")
    for i := 0; i < len(rows); i += batchSize {
        end := i + batchSize; if end > len(rows) { end = len(rows) }
        valueGroups := []string{}
        for j := i; j < end; j++ {
            row := rows[j]
            vals := make([]string, len(row))
            for k, v := range row { vals[k] = sg.formatValue(v) }
            valueGroups = append(valueGroups, fmt.Sprintf("(%s)", strings.Join(vals, ", ")))
        }
        if len(valueGroups) > 0 { sg.statements = append(sg.statements, fmt.Sprintf("INSERT INTO `%s` (%s) VALUES %s;", tableName, colList, strings.Join(valueGroups, ", "))) }
    }
}

func (sg *SqlGenerator) LogLoadDataLocalInfile(tableName string, columns []string, csvPath string) {
    if len(columns) == 0 || csvPath == "" { return }
    varCols := make([]string, len(columns))
    setClauses := make([]string, len(columns))
    for i, col := range columns { varName := fmt.Sprintf("@v%d", i); varCols[i] = varName; setClauses[i] = fmt.Sprintf("`%s` = NULLIF(%s, '')", col, varName) }
...
func (sg *SqlGenerator) LogCreateTempTable(tmpTable string, columns []string, idColumn string, df Frame) {
    ...
    for _, col := range columns {
        ...
        if col == idColumn { sqlType = "BIGINT" }
...
func (sg *SqlGenerator) LogUpdateFromTempJoin(targetTable, tmpTable, idColumn string, columnNames []string) {
    if len(columnNames) == 0 { return }
    sets := make([]string, 0, len(columnNames))
    for _, col := range columnNames { sets = append(sets, fmt.Sprintf("t.`%s`=u.`%s`", col, col)) }
...
func (sg *SqlGenerator) LogDropTempTable(tmpTable string) { sg.statements = append(sg.statements, fmt.Sprintf("DROP TEMPORARY TABLE IF EXISTS `%s`;", tmpTable)) }
...
func (sg *SqlGenerator) LogCreateTable(tableName string, df Frame, columns []string) {
    ...
    createSQL := fmt.Sprintf(`CREATE TABLE IF NOT EXISTS %s (
        ...
        %s,
        PRIMARY KEY (`+"`id`"+`)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin`, tableName, strings.Join(cols, ", "), strings.Join(indexes, ", "))
...
func (sg *SqlGenerator) LogCreateIndex(tableName string, columns []string, indexType string) {
    ...
    case "UNIQUE": sql = fmt.Sprintf("CREATE UNIQUE INDEX `%s` ON `%s` (%s)", indexName, tableName, columnList)
    case "FULLTEXT": sql = fmt.Sprintf("CREATE FULLTEXT INDEX `%s` ON `%s` (%s)", indexName, tableName, columnList)
    default: sql = fmt.Sprintf("CREATE INDEX `%s` ON `%s` (%s)", indexName, tableName, columnList)
    }
    sg.statements = append(sg.statements, sql)
}

func (sg *SqlGenerator) LogDropTable(tableName string) { sg.statements = append(sg.statements, fmt.Sprintf("DROP TABLE IF EXISTS `%s`", tableName)) }
func (sg *SqlGenerator) LogTruncateTable(tableName string) { sg.statements = append(sg.statements, fmt.Sprintf("TRUNCATE TABLE `%s`", tableName)) }
func (sg *SqlGenerator) LogAnalyzeTable(tableName string) { sg.statements = append(sg.statements, fmt.Sprintf("ANALYZE TABLE `%s` ALL COLUMNS", tableName)) }
func (sg *SqlGenerator) LogSetVariable(variable string, value interface{}) { sg.statements = append(sg.statements, fmt.Sprintf("SET %s = %s", variable, sg.formatValue(value))) }
func (sg *SqlGenerator) LogUseDatabase(databaseName string) { sg.statements = append(sg.statements, fmt.Sprintf("USE `%s`", databaseName)) }
func (sg *SqlGenerator) LogComment(comment string) { for _, line := range strings.Split(comment, "\n") { sg.statements = append(sg.statements, fmt.Sprintf("-- %s", strings.TrimSpace(line))) } }
func (sg *SqlGenerator) LogEmptyLine() { sg.statements = append(sg.statements, "") }
func (sg *SqlGenerator) Save(filename string) error { if len(sg.statements) == 0 { return nil }; return os.WriteFile(filename, []byte(strings.Join(sg.statements, "\n")), 0644) }
...
func (sg *SqlGenerator) inferSQLType(df Frame, colName string) string {
    ...
    if total == 0 { return "VARCHAR(255)" }
    if floatCount > total/2 { return "DOUBLE" } else if intCount > total/2 { return "BIGINT" } else if dateCount > total/2 { return "DATETIME" } else { return "VARCHAR(255)" }
}
